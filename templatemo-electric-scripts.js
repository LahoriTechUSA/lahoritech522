...
function createParticles() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        if (Math.random() > 0.5) {
            particle.style.setProperty('--particle-color', '#F3E5AB');
            particle.style.background = '#F3E5AB';
        } else {
            particle.style.background = '#D4AF37';
            particle.style.setProperty('--particle-color', '#D4AF37');
        }
...
document.querySelectorAll('.nav-links a').forEach(link => { link.addEventListener('click', () => { menuToggle.classList.remove('active'); navLinks.classList.remove('active'); }); });
